package net.fxug.sendai.study166.airkinect {
	...
	public class PointCloudRenderer extends Sprite {
		...
		private static const _matrix:Matrix3D = new Matrix3D();
		...
		public function PointCloudRenderer(settings:KinectSettings, maxDepth:uint = 2048) {
			init(settings, maxDepth);
		}

		public function init(settings:KinectSettings, maxDepth:uint = 2048):void {
			_maxDepth = maxDepth;
		...
		protected function mouseWheelHandler(event:MouseEvent):void {
			_targetZ -= event.shiftKey ? event.delta * 8 : event.delta;
		}
		...
		private function redraw():void {
			...
			var targetX:Number = ((( (mouseX / 4) / _explicitWidth) - .5)) * 90;
			var targetY:Number = ((( (mouseY / 4) / _explicitHeight) - .5)) * -90;
			...
			_pointCloudImage.width = 640;
			_pointCloudImage.height = 480;
...
import net.fxug.sendai.study166.airkinect.PointCloudRenderer;
